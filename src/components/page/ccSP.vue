	  ...
	  <el-table
	    ...
	    <el-table-column
	      ...
	      <template scope="scope">
	        ...
	        <p ><span>{{ scope.row.data[4]['key'] }} : </span><span>{{ scope.row.data[4]['value'] }}</span></p>
	      </template>
	    </el-table-column>
	  </el-table>
  ...
  export default {
    ...
    methods: {
    		...
    		if(this.value1 == '' ||this.value2 == ''){
				...
				function format2(shijianchuo){
					...
					return y+'-'+add0(m)+'-'+add0(d)+' 23:59:59';
	    				...
	    				axios.get('/admin/statics/12?pageNum=1&pageSize=10&beginTime='+this.value1+'&endTime='+this.value2).then( res =>{
	    					console.log(res)
    	...
    	handleCurrentChange(val){
	        ...
	        axios.get('/admin/statics/12?pageNum='+val+'&pageSize='+this.pageSize+'&beginTime='+this.value1+'&endTime='+this.value2).then( res =>{
    					for(var i=0;i<res.data.data.list.length;i++){
	    					res.data.data.list[i]['data'] = JSON.parse(res.data.data.list[i]['data']);
	    				}
    					this.tableData = res.data.data.list;
